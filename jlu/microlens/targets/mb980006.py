...
def make_cmd():
    work_dir = '/Users/jlu/work/microlens/MB980006/'
    
    acs_table = work_dir + 'hst_10198_0k_acs_hrc_total/hst_10198_0k_acs_hrc_total_daophot_trm.cat'
    wfpc2_table = work_dir + 'hst_08654_03_wfpc2_total_wf/hst_08654_03_wfpc2_total_wf_sexphot_trm.cat'
    ...
    # Align NIRC2 to ACS
    ##########
    print('Align NIRC2 Kp to ACS 814W')
    ...
    # Align WFPC2 to ACS
    ##########
    print('Align WFPC2 multi-band to ACS 814')
    ...
    plt.figure(1)
    plt.clf()
    plt.plot(nirc2_trans['x'], nirc2_trans['y'], 'rx', label='NIRC2')
    plt.plot(wfpc2_trans['x'], wfpc2_trans['y'], 'gs', mfc='none', mec='green', label='WFPC2')
    plt.plot(acs['x'], acs['y'], 'b+', label='ACS')
    plt.axis('equal')
    plt.xlabel('X (arcsec)')
    plt.ylabel('Y (arcsec)')
    plt.legend()
    ...
    plt.plot(vx, vy, 'ko', label='All')
    plt.plot(vx[idx], vy[idx], 'ro', label='r<2"')
    plt.plot(vx[ndx], vy[ndx], 'y*', ms=20, label='Source+Lens')
    plt.axis('equal')
    ...
    mag = acs_t_match['m_814']
    plt.plot(color, mag, 'k.', label='All')
    plt.plot(color[idx], mag[idx], 'r.', label='r<2"')
    plt.plot(color[ndx], mag[ndx], 'y*', ms=20, label='Source+Lens')
    plt.gca().invert_yaxis()
    plt.xlabel('m_814 - m_kp (mag)')
    plt.ylabel('m_814 (mag)')
    plt.legend()
    ...
    plt.plot(color, mag, 'k.', label='All')
    plt.plot(color[idx], mag[idx], 'r.', label='r<2"')
    plt.plot(color[wdx], mag[wdx], 'y*', ms=20, label='Source+Lens')
    plt.gca().invert_yaxis()
    ...
    plt.figure(7)
    plt.clf()
    color = wfpc2_match['m_675'] - wfpc2_match['m_814']
    mag = wfpc2_match['m_814']
    plt.plot(color, mag, 'k.', label='All')
    plt.plot(color[idx], mag[idx], 'r.', label='r<2"')
    plt.plot(color[wdx], mag[wdx], 'y*', ms=20, label='Source+Lens')
    plt.gca().invert_yaxis()
    plt.xlabel('m_675 - m_814 (mag)')
    plt.ylabel('m_814 (mag)')
    plt.xlim(0, 2)
    plt.legend()
    ...
    plt.figure(8)
    plt.clf()
    color = wfpc2_match['m_814'] - acs_t_match['m_814']
    mag = acs_t_match['m_814']
    plt.plot(color, mag, 'k.', label='All')
    plt.plot(color[idx], mag[idx], 'r.', label='r<2"')
    plt.plot(color[wdx], mag[wdx], 'y*', ms=20, label='Source+Lens')
    plt.gca().invert_yaxis()
    plt.xlabel('m_814 (WFPC2) - m_814 (ACS) (mag)')
    plt.ylabel('m_814 (mag)')
    #plt.xlim(0, 2)
    plt.legend()
