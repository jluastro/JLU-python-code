...
cat_dir = '/u/pboyle/work/data/catalogs/wd1_catalogs/membership_probabilities/box_3_std_vx_vy_NG_3_G1_False/'
catalog = 'catalog_probs_3.fits'
plot_dir = '/Users/jlu/work/wd1/analysis_2018_09_13/'
...
def plot_err_vs_mag(catalog, epoch, plot_dir=plot_dir):
    t = catalog
    
    # Fetch the epoch index
    ee = np.where(np.array(t.meta['EPNAMES']) == epoch)[0][0]

    x = t['x'][:, ee]
    y = t['y'][:, ee]
    m = t['m'][:, ee]
    xe = t['xe'][:, ee] * astrometry.scale['WFC'] * 1e3
    ye = t['ye'][:, ee] * astrometry.scale['WFC'] * 1e3
    me = t['me'][:, ee]

    good = (np.isnan(m) == False) & (np.isnan(x) == False) & (np.isnan(y) == False)
    x = x[good]
    y = y[good]
    m = m[good]
    xe = xe[good]
...
def map_of_errors(catalog, epoch, pos_err_cut=3.0):
    """
...
def plot_velerr_vs_mag(catalog, epoch_mag):
    """
    ...
    # Fetch the epoch index
    ee = np.where(np.array(t.meta['EPNAMES']) == epoch_mag)[0][0]
...
def make_cmd(catalog, cl_prob=0.3, usePcolor=False, suffix=''):
    """
    ...
    # Make a dictionary for the epoch indices by name:
    ee = dict(zip(d.meta['EPNAMES'], np.arange(len(d.meta['EPNAMES']))))

    ##########
    # Plot CMD with everything in black.
    ##########
    mag1 = d['m'][:, ee['2013_F160W']]
    mag2 = d['m'][:, ee['2005_F814W']]
    color = mag2 - mag1
...
def plot_color_color(catalog, cl_prob=0.6, usePcolor=True, suffix=''):
    # Read in data table
